...
def sbo_short(
    ...
    :param trend_filter: Tuple containing trend filter period and type (e.g., (20, 'ema') or (20, 'sma')).
    ...
    df = df.with_columns([
        pl.col('Close').rolling_min(window_size=X).alias('RollingMin')
    ])
    df = compute_effective_atr(df, X, atr_type)
    
    # Step 2: Calculate the Sell Threshold
    df = df.with_columns([
        (pl.col('RollingMin') - r * pl.col('ATR')).shift(1).alias('Sell_Threshold')
    ])

    # Calculate the moving average if trend filter is provided
    if trend_filter is not None:
        tf_period, tf_type = trend_filter
        if tf_type.lower() == 'ema':
            df = df.with_columns([
                pl.col('Close').ewm_mean(span=tf_period, adjust=False).alias('TrendFilter')
            ])
            df = df.with_columns([
                pl.col('TrendFilter').shift(1).alias('TrendFilter')
            ])
            df = df.with_columns([
                (pl.col('Close') < pl.col('TrendFilter')).alias('TrendFilter')
            ])
        elif tf_type.lower() == 'sma':
            df = df.with_columns([
                pl.col('Close').rolling_mean(window_size=tf_period).alias('TrendFilter')
            ])
            df = df.with_columns([
                pl.col('TrendFilter').shift(1).alias('TrendFilter')
            ])
        ...
        df = df.with_columns([
            pl.lit(True).alias('TrendFilter')
        ])

    # Step 3: Identify Sell Signals
    df = df.with_columns([
        ((pl.col('Close') < pl.col('Sell_Threshold')) & (pl.col('TrendFilter'))).alias('Sell_Signal')
    ])
    ...
    for i in range(X, len(df_list) - 1):
        if not position_open:
            ...
            if df_list[i]['Sell_Threshold'] is not None and df_list[i]['Close'] < df_list[i]['Sell_Threshold']:
                if df_list[i]['TrendFilter']:
            ...
            # Check for Buy Signal after holding period N to close the short position
            if i + 1 == entry_index + N:
                df_list[i + 1]['Signal'] = 1  # Buy signal to close short
    ...
    # Define the schema to ensure consistency when converting back to Polars
    schema = df.schema
...
def sbo_short_pb(
    ...
    :return: Polars DataFrame with trading signals appended to the original DataFrame.
    """
    # Step 1: Compute Effective ATR and Rolling Min
    df = df.with_columns([
        pl.col('Close').rolling_min(window_size=X).alias('RollingMin')
    ])
    df = compute_effective_atr(df, X, atr_type)
    ...
    # Step 3: Identify Sell Signals
    df = df.with_columns([
        (pl.col('Close') < pl.col('Sell_Threshold')).alias('Sell_Signal')
    ])
        ...
        if not position_open:
            ...
            if df_list[i]['Sell_Signal']:
                ...
                if not order_filled:
                    ...
                    pass  # Do nothing, wait for next Sell signal
        else:
    ...
    schema = {
        ...
        'RollingMin': pl.Float64,
        'ATR': pl.Float64,
        'Sell_Threshold': pl.Float64,
        'Sell_Signal': pl.Boolean,
        'Signal': pl.Int64,
        'TradeEntry': pl.Float64,
        'TradeExit': pl.Float64,
        'InTrade': pl.Int64,
...
def bo_rsi_short(
    ...
    while i < len(entry_indices):
        ...
        while exit_idx < len(df) - 1:
            ...
            exit_by_high = close[exit_idx] < low[exit_idx - 1]
...
def bo_rsi_short_ml(
    ...
    for idx in entry_indices:
        ...
        if available_dps >= min_dps:
            ...
            train_indices = [
                i for i in past_signals[:-1]
                if i + N + 1 <= idx
            ][-max_dps:]  # Get the last max_dps valid indices
            
            X_train = df.loc[train_indices, ml_feature_names]
            y_train = np.sign(df.loc[train_indices, 'mlfeat_result'])

            # Validate that no overlap exists (optional check)
            for i in train_indices:
                assert i + N + 1 <= idx, f"Lookahead detected: Training point {i} overlaps with test point {idx}"
            ...
            if len(y_train) >= min_dps:
                ...
                if X_test.notnull().all(axis=1).iloc[0]:
                    ...
                    try:
                        ...
                        prob = prob[0][0]
                    df.at[idx, 'mlfeat_hist_count'] = len(y_train)
        ...
        while exit_idx < len(df) - 1:
            ...
            if exit_quick_rule and days_in_trade > N_min:
                exit_by_low = close.iloc[exit_idx] < low.iloc[exit_idx - 1]
            else:
                exit_by_low = False
    ...
    df['TradeEntry'] = trade_entries
    df['TradeExit'] = trade_exits
    df['InTrade'] = in_trade.astype(int) * trade_sign * -1
