...
class TradingRule:
    def __init__(
        ...
        if incl_assets != []:
            ...
            self.incl_assets = list(market_data.data.keys())
            self.market_data = market_data
    ...
    def apply_position_sizing(self, df: pl.DataFrame, asset: str) -> pl.DataFrame:
        ...
        # Calculate the rolling 30-day standard deviation of changes in the close price
        df['20d_Price_Vol'] = df['Close'].diff().rolling(window=20).std()
        df['20d_Price_Vol'] = df['20d_Price_Vol'].shift(1).bfill()
        ...
        for trade_id, group in grouped:
            if trade_id:  # Exclude any rows where 'Trade_ID' might be missing
                ...
                summary = {
                    ...
                    'Trade_PnL': group['Strategy_PnL_USD'].sum(),
                    'Price_PnL': last_valid_trade_exit-first_valid_trade_entry,
                    '20d_Price_Vol': group['20d_Price_Vol'].iloc[0],
                    'Std_PnL': (last_valid_trade_exit-first_valid_trade_entry) * np.sign(first_row['Num_Lots']) / np.abs(group['20d_Price_Vol'].iloc[0]) if group['20d_Price_Vol'].iloc[0] != 0 else np.nan,
                }
    ...
    def plot_equity(self, byac: bool = False, byassets: bool = False, totalsys: bool = True,
                    ...
                    naming='descr', calc_metric='sr', plot_type: str = 'mpl'):
        """
        Plots the Strategy_Equity_USD using Matplotlib or Plotly and optionally saves the figure.
        ...
        :param excl_ac: List of asset classes to exclude.
        :param excl_assets: List of assets to exclude.
        ...
        :param force_bt: If True, forces backtesting of all assets.
        :param save_fig: If True, saves the figure to a file.
        :param file_format: The format to save the figure ('html', 'png', or 'svg').
        :param naming: Naming convention for the plot title ('descr' or 'params').
        :param calc_metric: Metric to calculate ('sr', 'other_metric').
        :param plot_type: Type of plot to generate ('mpl' for Matplotlib, 'plotly' for Plotly).
        ...
        if plot_type == 'mpl':
            ...
            for column in pnl_df.columns:
                ...
                ax.plot(pnl_df.index, pnl_df[column], label=column)
            ax.set_title(f'{plt_tit} Equity Curves')
            ax.set_xlabel('Date')
            ...
            if save_fig:
                ...
                if file_format.lower() in ['png', 'svg']:
                    plt.savefig(save_path, format=file_format.lower())
                    self.logger.info(f"Figure saved as '{save_path}'")
                else:
                    self.logger.error("Invalid file format for Matplotlib. Choose 'png' or 'svg'.")
            ...
            if save_fig:
                ...
                if file_format.lower() == 'html':
                    ...
                    self.logger.info(f"Figure saved as '{save_path}'")
                elif file_format.lower() in ['png', 'svg']:
                    try:
                    ...
                    self.logger.error("Invalid file format. Choose 'html', 'png', or 'svg'.")
        else:
            self.logger.error("Invalid plot_type. Choose 'mpl' or 'plotly'.")
            return

        return pnl_df

    def plot_equity_old(self, byac: bool = False, byassets: bool = False, totalsys: bool = True,
                    filter_assets: List[str] = [], filter_ac: List[str] = [],
                    excl_ac: List[str] = [], excl_assets: List[str] = [],
                    start_date: str = '', end_date: str = '', force_bt = False,
                    save_fig: bool = False, file_format: str = 'html',
    ...
    def _calculate_trade_statistics(self, column_name: str, asset: str = None, by_actual_trade: bool = True) -> Dict[str, Any]:
        ...
        num_wins = (trades_df['Trade_PnL'] > 0).sum()
        num_losses = (trades_df['Trade_PnL'] < 0).sum()

        # Hit Rate (%)
        hit_rate = (num_wins / num_trades) * 100 if num_trades > 0 else np.nan

        # Average Win and Loss
        avg_win = trades_df.loc[trades_df['Trade_PnL'] > 0, 'Trade_PnL'].mean()
        avg_loss = trades_df.loc[trades_df['Trade_PnL'] < 0, 'Trade_PnL'].mean()

        # Win/Loss Ratio
        win_loss_ratio = avg_win / -avg_loss if avg_loss != 0 else np.nan

        # Total PnL Winners and Losers
        total_pnl_winners = trades_df.loc[trades_df['Trade_PnL'] > 0, 'Trade_PnL'].sum()
        total_pnl_losers = trades_df.loc[trades_df['Trade_PnL'] < 0, 'Trade_PnL'].sum()
        ...
        pnl_skew = trades_df['Trade_PnL'].skew()

        # Trade Expected Value
        trade_ev = trades_df['Trade_PnL'].mean()
        ...
        # Percentiles
        percentile_25 = trades_df['Trade_PnL'].quantile(0.25)
        percentile_50 = trades_df['Trade_PnL'].quantile(0.5)
        percentile_75 = trades_df['Trade_PnL'].quantile(0.75)

        # Pareto Number (%)
        sorted_pnl = trades_df['Trade_PnL'].sort_values(ascending=False)
    ...
    def _full_tradeslist(self, filter_assets: List[str] = []):
        ...
        for file_path in parquet_files:
            ...
            if asset_name not in self.incl_assets:
                continue
